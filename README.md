...
- GPT-6 Sol / high is the default architecture-sensitive and end-to-end tier; GPT-6 Astra is reserved for explicit operator/reviewer escalation when concrete evidence justifies the higher-cost route;
...
The routing policy is grounded in production benchmarks. #93/#95 showed the cost separation between cheap mechanical work and stronger investigative models, while #139 showed that automatically routing end-to-end work to Astra could consume nearly an entire freshly reset five-hour allowance without a correspondingly unique implementation benefit. Terra is historical benchmark data rather than an active route; GPT-6 Sol now covers investigative, architecture-sensitive, and end-to-end work by default, with Astra reserved for explicit evidence-based escalation. See [`docs/PHASE2_BUDGETED_ROUTING.md`](docs/PHASE2_BUDGETED_ROUTING.md) for the measurements and routing table.
...
Workers can edit source in their GH workspace but use typed host interfaces for privileged/fragile seams. Unity execution does not broadly grant production-scene authority: existing scenes remain under mechanical/structural tiers, while the separately labeled new-scene lane is bound by host-owned provenance to one unmerged target scene and cannot edit a scene already present on `origin/main`. Git handoff validates workspace/repository/branch/history and never exposes a generic host command runner. Normal implementation stops at a reviewable PR; explicitly eligible report-only work stops at a host-verified `symphony:report-complete` issue state. Telemetry/activity/detail aggregation remains read-only with respect to RPG Kingdom/GitHub lifecycle; startup maintenance mutates only Supervisor-local telemetry and never grants model turns additional credentials or permissions. Merge and issue closure remain human decisions.
