...
- a host-side budget guard removes `symphony:ready` if a worker attempt ends without a clean PR handoff, preventing silent fresh-session redispatch;
...
- [`scripts/after-run-guard.sh`](scripts/after-run-guard.sh) — fail-closed redispatch guard.
...
Symphony is engineering-preview software and Codex App Server workers are intentionally autonomous. The configuration therefore keeps one concurrent worker, no automatic merge, a required PR review boundary, explicit model escalation, and a host-side execution-budget guard.
