...
async function loadOperations(){const [d,m]=await Promise.all([fetch('/api/operations',{cache:'no-store'}).then(r=>r.json()),fetch('/api/maintenance',{cache:'no-store'}).then(r=>r.json())]);const s=d.services||{},q=d.quota||{},rate=q.rateLimits||{},p=rate.primary||{},w=rate.secondary||{};let cards=['symphony','review','unity','git'].map(k=>serviceCard(k,s[k]||{})).join('');cards+=`<div class="card"><h2>Codex quota</h2>${rows(q,[['status','status'],['observedAt','sampled']])}<div class="row"><span class="muted">5h used / left</span><span>${esc(pct(p.usedPercent))} / ${esc(pct(p.remainingPercent))}</span></div><div class="row"><span class="muted">5h reset</span><span>${esc(p.resetsAtIso||'-')}</span></div><div class="row"><span class="muted">weekly used / left</span><span>${esc(pct(w.usedPercent))} / ${esc(pct(w.remainingPercent))}</span></div><div class="row"><span class="muted">weekly reset</span><span>${esc(w.resetsAtIso||'-')}</span></div>${q.reason?`<pre class="warn">${esc(q.reason)}</pre>`:''}</div>`;const active=d.activeWorkers||[];cards+=`<div class="card"><h2>Active Codex slots</h2>${active.length?active.map(a=>`<div class="test"><a class="worker-link" href="#" onclick="openWorker('${esc(a.runId)}');return false"><b>${esc(a.identifier)} · ${esc(a.role)}</b></a><div>${esc(a.model)} / ${esc(a.effort)}</div><div class="muted">${a.alive?'live':'stale'} · ${esc(a.startedAt)}</div></div>`).join(''):'<span class="muted">none</span>'}</div>`;cards+=`<div class="card"><h2>Telemetry retention</h2>${rows(m,[['retentionDays','retention days'],['observedAt','last maintenance'],['mode','mode']])}<div class="row"><span class="muted">stale reconciled</span><span>${esc((m.staleWorkers||[]).length)}</span></div><div class="row"><span class="muted">last pruned</span><span>${esc(JSON.stringify(m.pruned||{}))}</span></div>${m.reason?`<pre class="muted">${esc(m.reason)}</pre>`:''}</div>`;document.getElementById('operations').innerHTML=cards;document.getElementById('workers').innerHTML=(d.recentWorkers||[]).map(r=>`<tr onclick="openWorker('${esc(r.runId)}')"><td>${esc(r.identifier)}</td><td>${esc(r.role)}</td><td>${esc(r.model)} / ${esc(r.effort)}</td><td>${esc(r.outcome)}</td><td>${esc(workerTokens(r))}</td><td>${esc(workerDelta(r,'primary'))}</td><td>${esc(workerDelta(r,'secondary'))}</td><td>${esc(fmtDuration(r.durationSeconds))}</td></tr>`).join('')||'<tr><td colspan="8" class="muted">No completed worker telemetry yet.</td></tr>'}
...
function queueItem(i){const pr=i.prNumber?` · <a href="${esc(i.prUrl)}" target="_blank" rel="noreferrer">PR #${esc(i.prNumber)}</a>`:'';const cycle=i.reviewCycle?` · review ${esc(i.reviewCycle)}`:'';const repairs=i.repairAttempts!==null&&i.repairAttempts!==undefined?` · repairs ${esc(i.repairAttempts)}/${esc(i.maxRepairAttempts)}`:'';const verdict=i.latestVerdict?`<div class="meta">verdict: ${esc(i.latestVerdict)}${i.latestSummary?' · '+esc(i.latestSummary):''}</div>`:'';const reason=i.haltReason?`<div class="meta ${i.quotaBlocked?'warn':''}">reason: ${esc(i.haltReason)}</div>`:'';const head=i.headSha?` · ${esc(String(i.headSha).slice(0,12))}`:'';const worker=i.workerRunId?` · <a href="#" onclick="openWorker('${esc(i.workerRunId)}');return false">worker detail</a>`:'';return`<div class="queue-item"><div><a href="${esc(i.url)}" target="_blank" rel="noreferrer"><b>${esc(i.identifier)}</b></a> ${esc(i.title)}${pr}</div><div class="meta">${i.active?'active · ':'queued · '}${routeText(i)}${cycle}${repairs}${head}${worker} · ${fmtAge(i.stateAgeSeconds)} in state</div>${verdict}${reason}${i.humanActionRequired?'<div class="meta action">human action required</div>':''}</div>`}
...
async function openWorker(id){const r=await fetch('/api/worker/'+encodeURIComponent(id),{cache:'no-store'});if(!r.ok)return;const d=await r.json(),w=d.worker||{},l=d.continuationLineage||{},c=d.currentLifecycle||{};const unity=(d.unityRuns||[]).map(x=>`<div class="test"><a href="#" onclick="openRun('${esc(x.requestId)}');return false">${esc(x.requestId)}</a> · ${esc(x.operation)} · ${esc(x.finalStatus||x.status)}</div>`).join('')||'<span class="muted">none</span>';const git=(d.gitHandoffs||[]).map(x=>`<div class="test">${esc(x.title)} · ${esc(x.summary||x.status||'-')}</div>`).join('')||'<span class="muted">none</span>';const activity=(d.activity||[]).map(x=>`<div class="test"><b>${esc(fmtTime(x.observedAt))}</b> · ${esc(x.title||x.kind)}${x.summary?`<div class="meta">${esc(x.summary)}</div>`:''}</div>`).join('')||'<span class="muted">none</span>';const artifacts=(d.artifacts||[]).map(x=>`<div class="path">${esc(x.kind)} · ${esc(x.path)}</div>`).join('')||'<span class="muted">none</span>';document.getElementById('detail').innerHTML=`<h2>Worker ${esc(w.runId)}</h2><div class="grid"><div class="card"><h2>Lifetime</h2>${rows(w,[['identifier','issue'],['role','role'],['model','model'],['effort','effort'],['route','route'],['startedAt','started'],['endedAt','ended'],['durationSeconds','seconds'],['outcome','outcome']])}</div><div class="card"><h2>Lifecycle now</h2>${rows(c,[['lifecycleState','state'],['prNumber','PR'],['headSha','head'],['reviewCycle','review cycle'],['repairAttempts','repairs'],['haltReason','reason']])}</div><div class="card"><h2>Continuation lineage</h2>${rows(l,[['sequence','sequence'],['totalIssueLifetimes','issue lifetimes'],['previousRunId','previous'],['nextRunId','next']])}<pre class="muted">${esc(l.basis||'')}</pre></div><div class="card"><h2>Usage</h2><pre>${esc(JSON.stringify({tokenUsage:w.tokenUsage,quotaBefore:w.quotaBefore,quotaAfter:w.quotaAfter,quotaDelta:w.quotaDelta},null,2))}</pre></div><div class="card wide"><h2>Worker-window activity</h2>${activity}</div><div class="card"><h2>Unity runs</h2>${unity}</div><div class="card"><h2>Git handoffs</h2>${git}</div><div class="card wide"><h2>Artifacts / logs</h2>${artifacts}</div></div>`;document.getElementById('detail').classList.add('open');document.getElementById('detail').scrollIntoView({behavior:'smooth',block:'start'})}
