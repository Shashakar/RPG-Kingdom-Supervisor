...
async function refreshAll(){try{await Promise.all([loadOperations(),loadIssueViews()]);document.getElementById('stamp').textContent='updated '+new Date().toLocaleTimeString()}catch(e){document.getElementById('stamp').textContent='error: '+e}}
