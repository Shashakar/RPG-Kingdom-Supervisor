...
SKILL_ROOTS_TRANSFORM="$ROOT/scripts/patch-symphony-skill-roots.py"
...
for file in "$WORKFLOW" "$TRANSFORM" "$CONTINUATION_TRANSFORM" "$SKILL_ROOTS_TRANSFORM" "$RUNNER" "$APPLY" "$VERIFY"; do
  [[ -f "$file" ]] || {
...
grep -Fq 'python3 "$SKILL_ROOTS_TRANSFORM" "$SYMPHONY_ROOT"' "$APPLY" || {
  echo "symphony-permissions-compat-test: installer must apply the deterministic first-party skill-root transform" >&2
  exit 1
}
grep -Fq 'skills/extraRoots/set' "$SKILL_ROOTS_TRANSFORM" || {
  echo "symphony-permissions-compat-test: first-party skill transform must use the supported App Server extra-root API" >&2
  exit 1
}
grep -Fq '$rpgk-investigate-bug' "$SKILL_ROOTS_TRANSFORM" || {
  echo "symphony-permissions-compat-test: investigative route must explicitly select the first-party skill" >&2
  exit 1
}
...
grep -Fq 'skills/extraRoots/set' "$VERIFY" || {
  echo "symphony-permissions-compat-test: verifier must require first-party skill root registration" >&2
  exit 1
}
...
python3 -m py_compile "$TRANSFORM" "$CONTINUATION_TRANSFORM" "$SKILL_ROOTS_TRANSFORM"
bash -n "$APPLY"
bash -n "$VERIFY"
bash -n "$RUNNER"
