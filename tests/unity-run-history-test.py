...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="rpgk-unity-history-") as temp:
        ...
        # Unity can log a transient licensing error and then recover. That must not become the primary diagnosis.
        workspace, broker = request_fixture(root, "license-recovered")
        artifact = workspace / "Logs" / "SymphonyUnity" / "run-license-recovered"
        artifact.mkdir(parents=True)
        (artifact / "Editor.log").write_text(
            "Access token is unavailable; failed to update license token.\n"
            "Successfully updated license.\n"
            "Package Manager could not resolve a package before tests started.\n",
            encoding="utf-8",
        )
        response(
            broker, "license-recovered", status="failed", exitCode=1,
            stdout=f"RPG Kingdom Unity runner: artifacts -> {artifact}\n",
            stderr="Unity exited before test results were produced.",
        )
        ...
        recovered = runs["license-recovered"]
        assert recovered["diagnosis"]["category"] == "unity_runner"
        assert recovered["diagnosis"]["title"] != "Unity licensing failure"
        ...
        assert {item["requestId"] for item in filtered} == {"test-failure", "compile-failure", "license-recovered"}
